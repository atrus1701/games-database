...
class Admin_Add extends MY_Controller
{
	...
	var $page = 'add';
	var $title = 'Game Series >> Add Item';
	var $_item = array();
	...
	/**
	 * Display the add interface for the game series.
	 */
	function index()
	{
		if( $this->input->post() )
		...
		{
			// Get the item's data.
			$this->_item = $this->table_model->get_gameseries_for_admin_add();
		}
		
		// Store the game series and if there was an error.
	...
	/**
	 * Attempts to add the new game series into the database.
	 */
	function _process_post()
	{
		...
		$already_exists = $this->table_model->gameseries_exists(
			$this->input->post('name')
		);
		if( $this->has_error() ) return;
		
		// The game series already exists.
		if( $already_exists )
		{
			$this->set_results( FALSE, 'The Game Series "'.$this->input->post('name') . '" already exists.' );
			return;
		}

		// Add the game series.
		$result = $this->table_model->add_gameseries(
			$this->input->post('name'),
		...
		// Get the game series id auto-generated by the gameseries table.
		$id = $this->table_model->get_gameseries_id(
			$this->input->post('name')
		);
		if( $this->has_error() ) return;
		$this->add_content( 'id', $id );
		
		// The game series was successfully added.
		$this->set_results( TRUE, 'The Game Series "'.$this->input->post('name').'" has been added.' );
...
/* End of file gameseries/admin_add.php */
/* Location: ./application/controllers/gameseries/admin_add.php */
